<div>
    <div class="row">
        <div class="app-container">
            <div class="row mt-3">
                ...
                <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ $activeTab === 'gallerys' ? 'active' : '' }}" id="gallerys-tab"
                            data-bs-toggle="tab" data-bs-target="#gallerys" type="button" role="tab"
                            aria-controls="gallerys" aria-selected="{{ $activeTab === 'gallerys' ? 'true' : 'false' }}"
                            wire:click="setActiveTab('gallerys')">
                            Gallery
                        </button>
                    </li>
                    <li class="nav-item" role="presentation">
                ...
                <div class="tab-content" id="myTabContent">
                    <!-- Albums Tab -->
                    <div class="tab-pane fade {{ $activeTab === 'gallerys' ? 'show active' : '' }}" id="gallerys"
                        role="tabpanel" aria-labelledby="gallerys-tab">
                                        ...
                                        <div class="me-3">
                                            <div class="input-icon">
                                                <input type="text" class="form-control" placeholder="Search gallery..."
                                                    wire:model.debounce.300ms="searchGallery">
                                    ...
                                    <table class="table table-vcenter card-table table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>N. Of Foto Gallery</th>
                                                <th>Created At</th>
                                                <th class="w-1">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody id="sortable_gallery">
                                            @forelse ($Gallerys as $a)
                                            <tr data-index="{{ $a->id }}" data-ordering="{{ $a->ordering }}">
                                                <td>{{ ($Gallerys->currentPage() - 1) * $Gallerys->perPage() +
                                                    $loop->iteration }}
                                                </td>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a href="#" wire:click.prevent='editGallery({{ $a->id }})'
                                                            class="btn btn-sm btn-primary">Edit</a> &nbsp;
                                                        <a href="#" wire:click.prevent='deleteGallery({{ $a->id }})'
                                            ...
                                            @empty
                                            <tr>
                                                <td colspan="5"><span class="text-danger">Gallery Not
                                                        Found!</span>
                                                </td>
                                            </tr>
                                            @endforelse
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-auto ms-auto d-print-none">
                                    <div class="d-flex">
                                        <div class="me-3">
                                            <div class="input-icon">
                                                <select wire:model='gallery' id="" class="form-control">
                                                    <option value="">-- ALL foto --</option>
                                                    @foreach ($GalleryList as $al)
                                                    <option value="{{ $al->id }}">{{ $al->gallery_name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <a href="{{ route('gallery.add-foto.gallery') }}" class="btn btn-primary btn-sm"
                                            type="button">
                                        ...
                                        <button wire:click="selectAllFotoGallerys"
                    ...
                    <div class="page-body">
                        <div class="container-xl">
                            <div class="row row-cards">
                                @forelse ($fotosGallery as $foto)
                                <div class="col-sm-6 col-lg-4">
                                    <div class="card card-sm">
                                        <div class="position-relative">
                                            <input type="checkbox" wire:model="selectedFotoGallerys"
                                                value="{{ $foto->id }}" class="position-absolute top-0 start-0 m-2">
                                            <a href="{{ asset('storage/back/images/gallery/foto/' . $foto->image) }}"
                                                class="d-block" data-id="{{ $foto->id }}" data-fancybox="gallery"
                                                data-caption="{{ $foto->title }}">
                                                <img src="{{ asset('storage/back/images/gallery/foto/thumbnails/thumb_' . $foto->image) }}"
                                                    class="card-img-top">
                                            </a>
                                        </div>
                                        <div class="card-body">
                                            <div class="d-flex align-items-center">
                                                <div class="ms-auto">
                                                    <a wire:click.prevent='deleteFotoGallery({{ $foto->id }})' href=""
            ...
            {{-- Modals --}}
            <div wire:ignore.self class="modal fade" id="gallery_modal" tabindex="-1" role="dialog"
                aria-labelledby="myLargeModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="myLargeModalLabel">
                                {{ $updateGalleryMode ? 'Updated Gallery' : 'Add Gallery' }}
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form method="POST"
                            action="{{ $updateGalleryMode ? route('gallery.update', $selected_gallery_id) : route('gallery.store') }}"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter gallery name"
                                        name="gallery_name" value="{{ $gallery_name }}">
                                ...
                                <div class="mb-3">
                                    <label class="form-label">Gallery Thumbnail</label>
                                    <input type="file" class="form-control" placeholder="Enter gallery name"
                                        name="image">
                                    <span class="text-danger">
                                        @error('image')
                                        {!! $message !!}
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary">{{ $updateGalleryMode ? 'Update' : 'Save'
                                    }}</button>
                            </div>
                        </form>
                    </div>
            ...
            @if ($errors->has('gallery_name') || $errors->has('image'))
            <script>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="videoModalLabel">
                                {{ $updateVideoGalleryMode ? 'Update Video' : 'Add Video' }}
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form class="modal-content" method="POST" @if ($updateVideoGalleryMode)
                            wire:submit.prevent='updateVideo()' @else wire:submit.prevent='addVideo()' @endif>
                            <div class="modal-body">
                                @if ($updateVideoGalleryMode)
                            ...
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary">{{ $updateVideoGalleryMode ? 'Update' :
                                    'Save'
                                    }}</button>
                            </div>
                        </form>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                    document.getElementById('deleteAllButton').addEventListener('click', function() {
                        ...
                        if (confirm('Are you sure? You won\'t be able to revert this!')) {
                            ...
                            if (gallery) {
                                Livewire.emit('deleteSelectedFotoGallerys', gallery);
                            } else {
                                Livewire.emit('deleteSelectedFotoGallerys');
                            }
                        }
                    });
                });
