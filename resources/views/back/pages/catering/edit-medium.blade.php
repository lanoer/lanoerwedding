...
@section('pageTitle', isset($pageTitle) ? $pageTitle : 'Edit Medium Catering')


@section('pageHeader')
<div class="col-12">
    <div class="page-title-box d-flex align-items-center justify-content-between">
        <h4 class="page-title mb-0 font-size-18">Edit Medium Catering</h4>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title mb-0">Edit Medium Catering</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('catering.sub.updateMedium', $mediums->id) }}" method="POST"
                    ...
                    <div class="image_holder mb-2" style="max-width: 250px">
                        @if($mediums->image)
                        <img src="{{ asset('storage/back/images/catering/premium/' . $mediums->image) }}" alt=""
                    ...
                    <div class="form-group mb-3">
                        ...
                        <div class="row mt-2">
                            @foreach ($mediums->images as $img)
                            <div class="col-3 mb-2" id="gallery-img-{{ $img->id }}">
                                <img src="{{ asset('storage/back/images/catering/medium/gallery/thumbnails/thumb_800_' . $img->image) }}"
                    ...
                    <div class="form-group">
                        ...
                        <textarea class=" form-control @error('description') is-invalid @enderror" name="description"
                            rows="6" placeholder="Content.." id="description">{!! $mediums->description !!}</textarea>
                        @error('description')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group mb-3 mt-3">
                        ...
                        <input type="text" class="form-control @error('meta_description') is-invalid @enderror"
                            id="meta_description" name="meta_description" value="{{ $mediums->meta_description }}">
                    ...
                    <div class="form-group mb-3 mt-3">
                        ...
                        <input type="text" class="form-control @error('meta_keywords') is-invalid @enderror"
                            id="meta_keywords" name="meta_keywords" value="{{ $mediums->meta_keywords }}">
                        @error('meta_keywords')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group mb-3 mt-3">
                        ...
                        <input type="text" class="form-control @error('meta_tags') is-invalid @enderror" id="meta_tags"
                            name="meta_tags" value="{{ $mediums->meta_tags }}">
                        @error('meta_tags')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
...
<script>
    ...
    $('.delete-gallery-img').click(function() {
        ...
        if (confirm('Delete this image?')) {
            $.ajax({
                url: '{{ route('catering.delete.mediumGallery.image', ': id ') }}'.replace(':id', imgId),
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
<script>
    ...
    function uploadImage(file) {
        ...
        $.ajax({
            url: '/catering/sub/medium/upload-image', // Endpoint untuk upload gambar
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    function deleteImage(imageUrl) {
        ...
        $.ajax({
            url: '/catering/sub/medium/delete-image',
            type: 'POST',
            data: {
                imageUrl: imageUrl,
                _token: '{{ csrf_token() }}'
